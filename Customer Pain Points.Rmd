...
selected_demog <- demographics %>% select(ID, alter, geschlecht, prozent_berufstätig, wohnort, nutzung_oev, sbb_app, google_maps, neuen_ort,  aw_zug, aw_tram, fz_zug, störungen_oev) %>% 
   mutate(ID = as.numeric(ID))
...
```{r}
res_reg_cv_df$Label <- str_wrap(res_reg_cv_df$Question, width = 40)
...
res_reg_cv_df %>% 
   ...
   geom_col()+
   coord_flip()+
   #geom_point(aes(Label, rsq_max))+
   labs(title = "Out of sample explained variance in order of pain points")
```
